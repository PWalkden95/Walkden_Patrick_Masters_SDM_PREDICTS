...
coordinate <- data %>% spread(species, presence)  ### spread the data so that each row is the coordinates at while the presence is at
                                   ...
                                   #### evaluate how many occurrences occur at the same coordinates 
registerDoParallel(cores = 4)

coord_occ <- foreach (i = 1:nrow(coordinate), .combine= "rbind") %dopar% {                        ### quite a lot of coordinates overlap with a number of species 
  coord_occ <- sum(coordinate[i,c(3:591)], na.rm = TRUE)
}
       
hist(coord_occ, breaks = 15)                                                     ### looking at the histogram coord_occ is the number of species that share that coordinate
...
for(i in 1:length(coordinate)){                             #### so this is basically if there are more than 75 species recorded at a single coordinate
if(sum(coordinate[i,c(3:591)], na.rm = TRUE) > 75){        ### it should be marked down as an absence as we can assume that this coordinate has been more 
  for(j in 3:591){                                         ### comprehensively surveyed therefore more likely to be a tre absence.
...
save(file = "RData_European_Bee_Species_Occ_Absence_Data.RData", Occ_Absence_Data)
