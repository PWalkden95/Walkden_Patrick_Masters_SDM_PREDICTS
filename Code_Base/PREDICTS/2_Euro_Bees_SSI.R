...
##### THEN WITH ADRIANAS SSI CALC I CAN CALCULATE THE METRIC FOR MY BEES IN EUROPE
...
European_Bee_Diversity <- European_Bee_Diversity %>%
  mutate(LU_nvh = ifelse(grepl("primary",tolower(Predominant_habitat))|grepl("seconday", tolower(Predominant_habitat)),
                         "intact",
                         "converted"),
         LU_nvh = as.factor(LU_nvh),
         LU_nvh = relevel(LU_nvh, ref = "intact"))


European_Bee_SSI <- SSI_calc(European_Bee_Diversity)
save(European_Bee_SSI, file = "RData_European_Bee_LU_SSI.RData")
